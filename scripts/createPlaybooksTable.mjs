...
if (!serviceRoleKey) {
  console.error("Missing SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY environment variable.");
  process.exit(1);
}
...
async function ensureTable() {
  ...
  const ddl = `
    create table if not exists public."Playbooks" (
      ...
      constraint playbooks_project_unique unique (project_id)
    );
    alter table public."Playbooks" enable row level security;
    create policy if not exists "Allow service role inserts" on public."Playbooks"
      for insert using (auth.role() = 'service_role') with check (true);
    create policy if not exists "Allow service role reads" on public."Playbooks"
